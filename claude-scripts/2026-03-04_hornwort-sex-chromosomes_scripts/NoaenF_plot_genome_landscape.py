"""
Genome landscape: protein-coding genes + repeats for NoaenF
Nothoceros aenigmaticus female (collection Schafran 227)
...
Figure 2 (summary): five-panel gene and repeat summary
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
# ── Paths ──────────────────────────────────────────────────────────────────
GFF   = 'NoaenF_repeat_annotation.gff'
GTF   = 'NoaenF_gene_annotation.gtf'
FASTA = 'NoaenF_genome.fasta'
OUT   = 'NoaenF_genome_landscape'

SCAFFOLDS = ['Noaen.S1', 'Noaen.S2', 'Noaen.S3', 'Noaen.S4', 'Noaen.S5']
SLABELS   = ['S1', 'S2', 'S3', 'S4', 'S5']
WINDOW, STEP = 200_000, 50_000
SEX_CHR = 'Noaen.S5'   # putative U chromosome (smallest scaffold; unconfirmed)

# ── Colour palette ─────────────────────────────────────────────────────────
# S5 highlighted in terracotta as putative sex chromosome
...
# Nothoceros has no pararetrovirus in its repeat composition
...
print(f'  Total genome: {total_genome/1e6:.1f} Mb  ({len(genome)} sequences)')
...
for seqid in SCAFFOLDS:
    ...
    print(f'  {seqid}: {size/1e6:.1f} Mb  {d["n_genes"]:,} genes  '
          f'{d["rep_total"]:.1f}% repeat  {d["gene_dens"]:.1f} genes/Mb')

# ═══════════════════════════════════════════════════════════════════════════
# FIGURE 1 — Genome landscape tracks
# ═══════════════════════════════════════════════════════════════════════════
...
for i, seqid in enumerate(SCAFFOLDS):
    ...
    sex_note = '   [putative sex chromosome — unconfirmed]' if is_sex else ''
    lc       = '#7a2020' if is_sex else '#1a1a1a'
    ax.text(0.0, 1.04,
            f'S{i+1}  ·  {size:.1f} Mb  ·  {ngenes:,} genes  ·  {rep_pct:.0f}% repetitive{sex_note}',
            transform=ax.transAxes, fontsize=8,
            fontweight='bold', va='bottom', color=lc, clip_on=False)
...
fig1.text(0.5, 0.960,
          'Nothoceros aenigmaticus female (NoaenF)  —  Genome landscape',
...
# ═══════════════════════════════════════════════════════════════════════════
# FIGURE 2 — Summary panels (2 × 2 + E spanning)
# ═══════════════════════════════════════════════════════════════════════════
...
ax_E = fig2.add_subplot(gs2[2, :])   # repeat class breakdown, spans both columns
...
def bar_labels(ax, bars, vals, fmt='{:.1f}', dy_frac=0.02):
    ymax = ax.get_ylim()[1]
...
# ── A: Scatter — gene vs repeat coverage per window ───────────────────────
for seqid in SCAFFOLDS:
    wd = win_data[seqid]
...
def get_comp_d(seqid):
    s = summary[seqid]
    return [
        s['exon_pct'], s['intron_pct'], s['unannotated_pct'],
        s['LTR'], s['DNA'],
        s['LINE'] + s['MITE'],
        s['Unknown'],
    ]
...
# ── E: Repeat class breakdown stacked bar (spans both columns) ────────────
# Collect all detailed TE classes present across the 5 scaffolds
class_totals = defaultdict(float)
for seqid in SCAFFOLDS:
    size = genome[seqid]
    for cls, ivs in [(c, [(s, e) for s, e, sc in all_by_class[c]
                           if sc == seqid])
                     for c in all_by_class]:
        bp = merge_bp(ivs)
        if bp > 0:
            class_totals[cls] += bp / size * 100 / len(SCAFFOLDS)

# Group by major class, sort within each group by abundance
from itertools import groupby as _gb

def cls_sort_key(c):
    maj = get_major(c)
    order_map = {m: i for i, m in enumerate(MAJOR_ORDER)}
    return (order_map.get(maj, 99), -class_totals[c])

top_classes = [c for c in sorted(class_totals, key=cls_sort_key)
               if class_totals[c] > 0.05]   # only show classes > 0.05% avg

# Build per-scaffold values
E_vals = {}
for seqid in SCAFFOLDS:
    size = genome[seqid]
    E_vals[seqid] = {}
    for cls in top_classes:
        ivs = [(s, e) for s, e, sc in all_by_class[cls] if sc == seqid]
        E_vals[seqid][cls] = merge_bp(ivs) / size * 100

# Assign colours: shade within major class
import colorsys

def class_color(cls):
    maj  = get_major(cls)
    base = MAJOR_COLORS.get(maj, '#aaaaaa')
    # parse hex
    r = int(base[1:3], 16) / 255
    g = int(base[3:5], 16) / 255
    b = int(base[5:7], 16) / 255
    h, s, v = colorsys.rgb_to_hsv(r, g, b)
    # within a major class, vary lightness slightly
    same_maj = [c for c in top_classes if get_major(c) == maj]
    idx = same_maj.index(cls) if cls in same_maj else 0
    v2  = max(0.4, min(1.0, v + 0.08 * (len(same_maj) // 2 - idx)))
    r2, g2, b2 = colorsys.hsv_to_rgb(h, s, v2)
    return f'#{int(r2*255):02x}{int(g2*255):02x}{int(b2*255):02x}'

bottoms_e = np.zeros(len(SCAFFOLDS))
for cls in top_classes:
    vals = np.array([E_vals[s][cls] for s in SCAFFOLDS])
    ax_E.bar(x, vals, bw, bottom=bottoms_e,
             color=class_color(cls), label=cls.replace('/', '/\u200b'),
             linewidth=0, zorder=3)
    bottoms_e += vals

ax_E.set_xticks(x)
ax_E.set_xticklabels(SLABELS, fontsize=8)
ax_E.set_ylabel('% of scaffold', fontsize=8)
ax_E.set_xlim(-0.55, len(SCAFFOLDS) - 0.45)
style_ax(ax_E)
ax_E.set_title('Repeat class breakdown', fontsize=8.5, pad=5)
ax_E.text(-0.07, 1.10, 'E', transform=ax_E.transAxes,
          fontsize=11, fontweight='bold', va='top')
ax_E.legend(fontsize=6.5, ncol=5,
            loc='upper right', frameon=False,
            handlelength=0.9, handletextpad=0.4, columnspacing=0.8)

fig2.text(0.5, 0.975,
          'Nothoceros aenigmaticus female (NoaenF)  —  Gene and repeat summary',
