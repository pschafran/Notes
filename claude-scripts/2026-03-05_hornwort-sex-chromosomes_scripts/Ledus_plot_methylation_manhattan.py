"""
Whole-genome methylation comparison: LedusF (female, JC2) vs LedusM (male, H23)
...
Steps:
  ...
  5. Per-site coverage and genomic context analysis by methylation category
  6. Figure 1: Miami plot — three stacked panels sharing x:
  ...
  7. Figure 2: Correlation scatter — F vs M 5mC at homologous 200 kb windows
  8. Figure 3: Coverage distributions + genomic context by methylation category

NOTE: LedusF.S3 is the U chromosome (not S5!); LedusM.S5 is the V chromosome.
      LedusF autosomes: S1, S2, S4, S5
      LedusM autosomes: S1, S2, S3, S4

Outputs (sex_chromosome_analyses/):
  LedusF_LedusM_methylation_manhattan.pdf/png
  LedusF_LedusM_methylation_manhattan_correlation.pdf/png
  LedusF_LedusM_methylation_context.pdf/png
  wga/LedusF_LedusM_autosomes.paf  (cached alignment)
"""
...
# ── Paths ────────────────────────────────────────────────────────────────────
BASE   = '/media/data/projects/hornwort_sex_chromosomes/analysis/Leiosporoceros'
BASE_F = f'{BASE}/LeiosporocerosJC2'
BASE_M = f'{BASE}/LeiosporocerosH23'

FASTA_F  = f'{BASE_F}/final_genome_prep/LedusF_genome.fasta'
FASTA_M  = f'{BASE_M}/final_genome_prep/LedusM_genome.fasta'
METHYL_F = f'{BASE_F}/methylation/LedusF_genome.5mCG_5hmCG.bed'
METHYL_M = f'{BASE_M}/methylation/LedusM_genome.5mCG_5hmCG.bed'
...
GFF_M = f'{BASE_M}/final_genome_prep/pilon.4_renamed.fasta.mod.EDTA.TEanno.gff3'
GTF_M = f'{BASE_M}/final_genome_prep/braker_renamed.gtf'

MINIMAP2  = '/home/peter/bin/minimap2'
WGA_DIR   = 'wga'
PAF       = f'{WGA_DIR}/LedusF_LedusM_autosomes.paf'
AUTO_F_FA = f'{WGA_DIR}/LedusF_autosomes.fasta'
AUTO_M_FA = f'{WGA_DIR}/LedusM_autosomes.fasta'
OUT       = 'LedusF_LedusM_methylation_manhattan'

# ── Scaffold lists ───────────────────────────────────────────────────────────
# IMPORTANT: LedusF.S3 = U chromosome; autosomes are S1, S2, S4, S5
F_AUTO   = ['LedusF.S1', 'LedusF.S2', 'LedusF.S4', 'LedusF.S5']
M_AUTO   = ['LedusM.S1', 'LedusM.S2', 'LedusM.S3', 'LedusM.S4']
F_SEXCHR = 'LedusF.S3'   # U chromosome
M_SEXCHR = 'LedusM.S5'   # V chromosome
F_ALL    = F_AUTO + [F_SEXCHR]
M_ALL    = M_AUTO + [M_SEXCHR]

SCAF_SIZES = {
    'LedusF.S1': 35_479_792, 'LedusF.S2': 35_194_583,
    'LedusF.S3': 22_033_833,
    'LedusF.S4': 20_435_939, 'LedusF.S5': 18_842_051,
    'LedusM.S1': 36_040_362, 'LedusM.S2': 35_569_410,
    'LedusM.S3': 19_857_507, 'LedusM.S4': 18_700_947,
    'LedusM.S5':  5_298_153,
}
...
# ── Colour scheme ────────────────────────────────────────────────────────────
F_COL   = '#3d6e9e'   # female autosome 5mC
M_COL   = '#3a7a52'   # male autosome 5mC
U_COLOR = '#b87070'   # U sex chromosome (female)
V_COLOR = '#c47a4a'   # V sex chromosome (male)

CHROM_COLS = {
    'LedusF.S1': '#90b8d4', 'LedusF.S2': '#6090b8',
    'LedusF.S4': '#90c4a4', 'LedusF.S5': '#5a9070',
}
...
def context_fracs(mask, is_gene, rep_cls):
    ...
    for maj in MAJOR_ORDER:
        ...
        out[label] = float((sub_gene & (sub_cls == maj)).sum()) / n
    for maj in MAJOR_ORDER:
...
def load_meth(bed_file, scaffolds, min_cov=METHYL_MIN_COV):
    ...
    with open(bed_file) as fh:
        for line in fh:
            ...
            seqid = p[0]
            if seqid not in scset or int(p[9]) < min_cov: continue
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 5 — Sex chromosome and full autosomal windowed 5mC
# ══════════════════════════════════════════════════════════════════════════════
...
print('\nLoading female landscape annotations (S1, S2, S4, S5, S3=U)...')
...
for glabel, scaffolds, methyl_file, gff_file, gtf_file in [
    ('Female (LedusF)', F_AUTO, METHYL_F, GFF_F, GTF_F),
    ('Male (LedusM)',   M_AUTO, METHYL_M, GFF_M, GTF_M),
]:
    print(f'\n  {glabel}')
    ...
    for cat in CATS:
        ...
        fi = (~sub_g & ~sub_r).sum() / n * 100
        fg = sub_g.sum() / n * 100
        fr = sub_r.sum() / n * 100
        fb = (sub_g & sub_r).sum() / n * 100
        print(f'  {cat:16s}  {n:>10,}  {fi:>10.1f}%  {fg:>9.1f}%  {fr:>9.1f}%  {fb:>11.1f}%')
...
fig1.text(0.5, 0.97,
          'Leiosporoceros dussii — DNA methylation and genome composition (200 kb windows)',
...
auto_labels_map = {'LedusF.S1': 'S1', 'LedusF.S2': 'S2',
                   'LedusF.S4': 'S4', 'LedusF.S5': 'S5'}
...
chrom_patches = [mpatches.Patch(color=CHROM_COLS[s],
                                label=f'{auto_labels_map[s]}  ({SCAF_SIZES[s]/1e6:.0f} Mb)')
                 for s in F_AUTO]
...
ax2.set_title('Methylation correlation at syntenic 200 kb windows\n'
              'Leiosporoceros dussii  ·  autosomes only', fontsize=9, pad=8)
...
genome_labels = ['Female (LedusF)', 'Male (LedusM)']
...
for col, glabel in enumerate(genome_labels):
    ...
    parts_data = [np.log10(res['cov_data'][cat]) for cat in cats_present]
...
fig3.text(0.5, 0.975, 'A  Coverage by methylation category', ha='center', va='top',
...
fig3.text(0.5, 0.495, 'B  Genomic context by methylation category', ha='center', va='top',
          fontsize=10, fontweight='bold')

ctx_out = 'LedusF_LedusM_methylation_context'
