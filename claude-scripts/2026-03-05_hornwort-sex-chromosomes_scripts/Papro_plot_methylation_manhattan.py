"""
Whole-genome methylation comparison: PaproF (female) vs PaproM (male)

Steps:
  1. Extract autosomes → run minimap2 asm5 whole-genome alignment (if not cached)
  2. Load 5mC methylation for all scaffolds (autosomes + sex chromosomes)
  3. Compute 100 kb windowed 5mC at syntenic positions (PAF-guided)
  4. Load repeat/gene landscape annotations for genome track
  5. Figure 1: Miami plot — three stacked panels sharing x:
       top:    female 5mC (0 at bottom, 100 at top)
       middle: stacked-area genome landscape tracks (to scale)
       bottom: male 5mC (0 at top, 100 at bottom — outward from landscape)
  6. Figure 2: Correlation scatter — F vs M 5mC at homologous 100 kb windows

Outputs (sex_chromosome_analyses/):
  PaproF_PaproM_methylation_manhattan.pdf/png
  PaproF_PaproM_methylation_manhattan_correlation.pdf/png
  wga/PaproF_PaproM_autosomes.paf  (cached alignment)
"""
import bisect
import math
import os
...
FASTA_F  = f'{BASE_F}/final_genome_prep/pilon.3_renamed.chromosomes.fasta'
FASTA_M  = f'{BASE_M}/final_genome_prep/PaproM.renamed_renamed.chromosomes.fasta'
...
WGA_DIR   = 'wga'
PAF       = f'{WGA_DIR}/PaproF_PaproM_autosomes.paf'
AUTO_F_FA = f'{WGA_DIR}/PaproF_autosomes.fasta'
AUTO_M_FA = f'{WGA_DIR}/PaproM_autosomes.fasta'
OUT       = 'PaproF_PaproM_methylation_manhattan'
...
SCAF_SIZES = {
    'PaproF.S1': 48_924_384,  'PaproF.S2': 45_184_183,
    'PaproF.S3': 39_397_387,  'PaproF.S4': 30_927_450,
    'PaproF.S5':  8_346_746,
    'PaproM.S1': 48_988_299,  'PaproM.S2': 44_767_416,
    'PaproM.S3': 40_070_168,  'PaproM.S4': 30_808_786,
    'PaproM.S5':  5_908_391,
}
...
# ── Colour scheme ──────────────────────────────────────────────────────────
F_COL   = '#3d6e9e'   # female autosome 5mC (deeper blue)
M_COL   = '#3a7a52'   # male autosome 5mC (deeper green)
U_COLOR = '#b87070'   # U sex chromosome (female)
V_COLOR = '#c47a4a'   # V sex chromosome (male)

CHROM_COLS = {
    'PaproF.S1': '#90b8d4', 'PaproF.S2': '#6090b8',
    'PaproF.S3': '#90c4a4', 'PaproF.S4': '#5a9070',
}

# Landscape stacked-area colours (same scheme as individual landscape figures)
MAJOR_ORDER  = ['LTR', 'DNA', 'LINE', 'MITE', 'pararetrovirus', 'Unknown']
MAJOR_COLORS = {
    'LTR':           '#c47a7a', 'DNA':           '#7aab8a',
    'LINE':          '#8a8ab8', 'MITE':          '#c8a86e',
    'pararetrovirus':'#9aacb8', 'Unknown':       '#7aaabf',
}
GENE_COLOR  = '#5a87a5'
UNANN_COLOR = '#dcdcdc'
STACK_COLORS = [GENE_COLOR, UNANN_COLOR] + [MAJOR_COLORS[m] for m in MAJOR_ORDER]
STACK_LABELS = ['Genes', 'Unannotated', 'LTR', 'DNA', 'LINE',
                'MITE', 'Pararetrovirus', 'Unknown repeat']

# ══════════════════════════════════════════════════════════════════════════
# Helper functions
# ══════════════════════════════════════════════════════════════════════════

# ── Statistics (pure NumPy — scipy unavailable) ───────────────────────────
def pearsonr(x, y):
...
if not os.path.exists(PAF):
    ...
    cmd = [MINIMAP2, '-x', 'asm5', '-t', '16', AUTO_F_FA, AUTO_M_FA, '-o', PAF]
...
with open(PAF) as fh:
    for line in fh:
        ...
        mapq = int(p[11])
        if mapq < MIN_MAPQ or (r_end - r_start) < MIN_BLOCK_LEN:
            continue
        if r_name not in F_AUTO or q_name not in M_AUTO:
            continue
...
def load_meth(bed_file, scaffolds, min_cov=5):
    ...
    with open(bed_file) as fh:
        for line in fh:
            ...
            if len(p) < 11 or p[3] != 'm':
                continue
            seqid = p[0]
            if seqid not in scset or int(p[9]) < min_cov:
                continue
            raw[seqid][0].append(int(p[1]))
            raw[seqid][1].append(float(p[10]))
    out = {}
    for s in scaffolds:
...
print('  Female...')
meth_f = load_meth(METHYL_F, F_ALL, METHYL_MIN_COV)
print('  Male...')
meth_m = load_meth(METHYL_M, M_ALL, METHYL_MIN_COV)

# ══════════════════════════════════════════════════════════════════════════
# STEP 4 — Syntenic 100 kb windows
# ══════════════════════════════════════════════════════════════════════════
...
for (r_name, r_start, r_end, q_name, q_start, q_end, strand) in blocks:
    ...
    for i in range(n_wins):
        ...
        if strand == '+':
            mw_s = q_start + int(frac_s * q_len)
            mw_e = q_start + int(frac_e * q_len)
        else:
            mw_s = q_end - int(frac_e * q_len)
            mw_e = q_end - int(frac_s * q_len)
        m_5mC = win_meth(*meth_m[q_name], mw_s, mw_e)
        if not (np.isnan(f_5mC) or np.isnan(m_5mC)):
            syn_r_name.append(r_name)
            syn_f_center.append((fw_s + fw_e) / 2)
            syn_f_5mC.append(f_5mC)
            syn_m_5mC.append(m_5mC)
...
# ══════════════════════════════════════════════════════════════════════════
# STEP 5 — Sex chromosome windowed 5mC
# ══════════════════════════════════════════════════════════════════════════
def scaffold_windows(meth_dict, scaf, size, window=WINDOW):
    ...
    for ws in starts:
        ...
        centers.append((ws + we) / 2)
        vals.append(win_meth(pos_arr, met_arr, ws, we))
...
# All female autosomal windows (for full coverage in female panel)
...
# Syntenic window x positions (Mb)
syn_x_mb = np.array([
    (f_auto_offset[r] + c) / 1e6
    for r, c in zip(syn_r_name, syn_f_center)
])

# Female autosomal: all windows
f_auto_x_mb = []
f_auto_y    = []
for s in F_AUTO:
    off = f_auto_offset[s]
    ...
    for c, v in zip(ctr, val):
        if not np.isnan(v):
            f_auto_x_mb.append((off + c) / 1e6)
            f_auto_y.append(v)
f_auto_x_mb = np.array(f_auto_x_mb)
f_auto_y    = np.array(f_auto_y)
...
def load_landscape_windows(gff_file, gtf_file, scaffolds, scaf_sizes, window=WINDOW):
    """Parse EDTA GFF3 + BRAKER GTF; return windowed stacked composition."""
    ...
    with open(gff_file) as fh:
        for line in fh:
            ...
            rep_by_seq[p[0]][get_major(cls)].append((int(p[3]), int(p[4])))
    ...
    # Merge intervals and compute windows
    win_data = {}
    for seqid in scaffolds:
        merged_rep  = {maj: merge_intervals(rep_by_seq[seqid][maj])
        ...
        merged_gene = merge_intervals(gene_ivs)
...
print('\nLoading female landscape annotations (S1–S5)...')
...
gs = gridspec.GridSpec(
    ...
    height_ratios=[2, 3, 2],
    hspace=0.04,
    top=0.88, bottom=0.09, left=0.055, right=0.98,
)
ax_f    = fig1.add_subplot(gs[0])
ax_land = fig1.add_subplot(gs[1], sharex=ax_f)
ax_m    = fig1.add_subplot(gs[2], sharex=ax_f)

# ── Shared visual elements: chromosome section shading ────────────────────
def shade_sections(ax):
    for i, s in enumerate(F_AUTO):
        x0 = f_auto_offset[s] / 1e6
        x1 = (f_auto_offset[s] + SCAF_SIZES[s]) / 1e6
        col = '#f7f7f7' if i % 2 == 0 else '#eeeeee'
        ax.axvspan(x0, x1, color=col, alpha=1.0, zorder=0)
...
shade_sections(ax_f)
shade_sections(ax_land)
shade_sections(ax_m)

# Thin vertical separators at scaffold boundaries
sep_positions = []
for s in F_AUTO:
    sep_positions.append(f_auto_offset[s] / 1e6)
sep_positions.append((f_auto_offset[F_AUTO[-1]] + SCAF_SIZES[F_AUTO[-1]]) / 1e6)
sep_positions.append(u_offset_bp / 1e6)
sep_positions.append((u_offset_bp + SCAF_SIZES[F_SEXCHR]) / 1e6)
sep_positions.append(v_offset_bp / 1e6)

for ax in [ax_f, ax_land, ax_m]:
    for xp in sep_positions:
        ax.axvline(xp, color='#c0c0c0', lw=0.5, zorder=1)

# ── TOP PANEL: female 5mC ─────────────────────────────────────────────────
# All female autosomal windows (background, lighter)
ax_f.scatter(f_auto_x_mb, f_auto_y,
             c=F_COL, s=5, alpha=0.35, linewidths=0, zorder=3, rasterized=True)
# Syntenic female windows (slightly more opaque, on top)
ax_f.scatter(syn_x_mb, syn_f_5mC,
             c=F_COL, s=5, alpha=0.65, linewidths=0, zorder=4, rasterized=True)
# U chromosome
valid_u = ~np.isnan(u_mC)
ax_f.scatter(u_x_mb[valid_u], u_mC[valid_u],
             c=U_COLOR, s=8, alpha=0.9, linewidths=0, zorder=5)
...
# U chr mean annotation
u_mid_mb = (u_offset_bp + SCAF_SIZES[F_SEXCHR] / 2) / 1e6
ax_f.text(u_mid_mb, u_mean + 2, f'{u_mean:.0f}%',
...
# ── MIDDLE PANEL: genome landscape ───────────────────────────────────────
# Female autosomes + U chr: female landscape
for seqid in F_ALL:
    wd  = land_f[seqid]
...
ax_land.set_ylim(0, 100)
ax_land.set_yticks([0, 50, 100])
...
ax_land.spines['bottom'].set_visible(False)
ax_land.spines['right'].set_visible(False)

# ── BOTTOM PANEL: male 5mC (y-axis inverted: 0 at top, 100 at bottom) ────
ax_m.scatter(syn_x_mb, syn_m_5mC,
             c=M_COL, s=5, alpha=0.65, linewidths=0, zorder=4, rasterized=True)
valid_v = ~np.isnan(v_mC)
ax_m.scatter(v_x_mb[valid_v], v_mC[valid_v],
             c=V_COLOR, s=8, alpha=0.9, linewidths=0, zorder=5)
...
# V chr mean annotation
v_mid_mb = (v_offset_bp + SCAF_SIZES[M_SEXCHR] / 2) / 1e6
ax_m.text(v_mid_mb, v_mean + 2, f'{v_mean:.0f}%',
          ha='center', va='bottom', fontsize=8, color=V_COLOR, fontweight='bold')

ax_m.set_ylim(0, 105)
ax_m.invert_yaxis()   # 0 at top (adjacent to landscape), 100 at bottom
...
xtick_pos, xtick_lab = [], []
for i, s in enumerate(F_AUTO):
    xtick_pos.append((f_auto_offset[s] + SCAF_SIZES[s] / 2) / 1e6)
    xtick_lab.append(f'S{i+1}\n{SCAF_SIZES[s]/1e6:.0f} Mb')
xtick_pos.append((u_offset_bp + SCAF_SIZES[F_SEXCHR] / 2) / 1e6)
xtick_lab.append(f'U\n{SCAF_SIZES[F_SEXCHR]/1e6:.1f} Mb')
xtick_pos.append((v_offset_bp + SCAF_SIZES[M_SEXCHR] / 2) / 1e6)
xtick_lab.append(f'V\n{SCAF_SIZES[M_SEXCHR]/1e6:.1f} Mb')

ax_m.set_xticks(xtick_pos)
ax_m.set_xticklabels(xtick_lab, fontsize=8.5)
ax_m.tick_params(axis='x', length=0, pad=4)
ax_m.set_xlim(-1, total_mb + 1)

# ── Section labels above top panel ────────────────────────────────────────
y_label = 1.10
ax_f.text(auto_total_bp / 2 / 1e6, y_label, 'Autosomes',
...
ax_f.annotate('', xy=(0, 1.06), xytext=(auto_total_bp / 1e6, 1.06),
              ...
              textcoords=ax_f.get_xaxis_transform(),
              arrowprops=dict(arrowstyle='-', color='#888888', lw=0.8))
ax_f.text((u_offset_bp + SCAF_SIZES[F_SEXCHR] / 2) / 1e6, y_label, 'U chr',
          transform=ax_f.get_xaxis_transform(),
          ha='center', va='bottom', fontsize=9.5, color=U_COLOR)
ax_f.text((v_offset_bp + SCAF_SIZES[M_SEXCHR] / 2) / 1e6, y_label, 'V chr',
          transform=ax_f.get_xaxis_transform(),
          ha='center', va='bottom', fontsize=9.5, color=V_COLOR)

# ── Legend ────────────────────────────────────────────────────────────────
land_patches = [mpatches.Patch(color=c, label=l, linewidth=0)
                for c, l in zip(STACK_COLORS, STACK_LABELS)]
meth_elems = [
    mpatches.Patch(color=F_COL,   label=f'♀ 5mC (autosomes, {f_auto_mean:.0f}% mean)'),
    mpatches.Patch(color=M_COL,   label=f'♂ 5mC (autosomes, {m_auto_mean:.0f}% mean)'),
...
# Landscape legend inside landscape panel (upper right)
ax_land.legend(handles=land_patches, fontsize=6.5, ncol=4,
               loc='upper right', frameon=True, framealpha=0.9,
               edgecolor='none', handlelength=0.9, handletextpad=0.4,
               columnspacing=0.8, borderpad=0.4)

# Methylation legend above figure
ax_f.legend(handles=meth_elems, fontsize=7.5, ncol=4,
            loc='lower center', frameon=False,
            handlelength=0.9, handletextpad=0.4, columnspacing=1.0,
            bbox_to_anchor=(0.5, 1.22))

fig1.text(0.5, 0.97,
          'Paraphymatoceros proskaueri — DNA methylation and genome composition (200 kb windows)',
          ha='center', va='top', fontsize=11, fontweight='bold', color='#1a1a1a')
fig1.text(0.5, 0.955,
          'Female (♀) above  ·  Male (♂) below  ·  Male autosomal windows at syntenic female position  ·  Nvalid_cov ≥ 5',
...
pt_colors = [CHROM_COLS[s] for s in syn_r_name]
ax2.scatter(syn_f_5mC, syn_m_5mC,
            c=pt_colors, s=7, alpha=0.5, linewidths=0, zorder=3, rasterized=True)
...
ax2.set_xlabel('Female 5mC per 200 kb window (%)', fontsize=9)
ax2.set_ylabel('Male 5mC per 200 kb window (%)', fontsize=9)
ax2.set_xlim(0, 100)
ax2.set_ylim(0, 100)
...
chrom_patches = [mpatches.Patch(color=CHROM_COLS[s],
                                 label=f'{s.replace("PaproF.", "")}  ({SCAF_SIZES[s]/1e6:.0f} Mb)')
                 for s in F_AUTO]
...
ax2.set_title('Methylation correlation at syntenic 200 kb windows\n'
              'Paraphymatoceros proskaueri  ·  autosomes only',
              fontsize=9, pad=8)
...
CTX_COLORS = {
    ...
    'gene+para':      '#9aacb8', 'gene+Unknown': '#7aaabf',
    'LTR':            '#c47a7a', 'DNA':          '#5a9070',
    'LINE':           '#6a6a98', 'MITE':         '#b08848',
    'pararetrovirus': '#7a9ca8', 'Unknown':      '#5a8a9f',
}
...
def load_genes_ctx(gtf_file, scaffolds):
    scaf_set = set(scaffolds)
...
def load_repeats_ctx(gff_file, scaffolds):
    scaf_set = set(scaffolds)
    ...
    with open(gff_file) as fh:
        for line in fh:
            ...
            attrs = {k: v for a in p[8].split(';') if '=' in a
                     for k, v in [a.split('=', 1)]}
            cls   = attrs.get('Classification', 'Unknown')
            reps[p[0]][get_major(cls)].append((int(p[3]), int(p[4])))
    for s in reps:
        for maj in reps[s]:
            reps[s][maj].sort()
    return dict(reps)
...
def context_fracs(mask, is_gene, rep_cls):
    in_rep = rep_cls != ''
    sub_gene = is_gene[mask]; sub_rep = in_rep[mask]; sub_cls = rep_cls[mask]
    n = mask.sum()
    if n == 0: return {}
    out = {'intergenic': float((~sub_gene & ~sub_rep).sum()) / n,
           'gene_only':  float((sub_gene & ~sub_rep).sum()) / n}
    for maj in MAJOR_ORDER:
        lbl = 'gene+' + ('para' if maj == 'pararetrovirus' else maj)
        out[lbl] = float((sub_gene & (sub_cls == maj)).sum()) / n
    for maj in MAJOR_ORDER:
        out[maj] = float((~sub_gene & (sub_cls == maj)).sum()) / n
    return out
...
# ══════════════════════════════════════════════════════════════════════════
# STEP 9 — Per-site context and coverage analysis
# ══════════════════════════════════════════════════════════════════════════
print('\nRunning per-site coverage and context analysis...')

ctx_results = {}
_genome_labels = ['Female (PaproF)', 'Male (PaproM)']
...
for glabel, scaffolds, methyl_file, gff_file, gtf_file in [
    ...
    genes_ctx = load_genes_ctx(gtf_file, scaffolds)
    reps_ctx  = load_repeats_ctx(gff_file, scaffolds)
    ...
    for s in scaffolds:
        ...
        pct_list.append(pct); gene_list.append(ig); rep_list.append(rc)

    all_cov  = np.concatenate(cov_list); all_pct  = np.concatenate(pct_list)
    all_gene = np.concatenate(gene_list); all_rep  = np.concatenate(rep_list)
...
# ══════════════════════════════════════════════════════════════════════════
# FIGURE 3 — Coverage + genomic context by methylation category
# ══════════════════════════════════════════════════════════════════════════
print('\nPlotting Figure 3: context and coverage...')
...
ctx_out = 'PaproF_PaproM_methylation_context'
...
def _rep_key(maj):
    return 'gene+' + ('para' if maj == 'pararetrovirus' else maj)

stats_path = 'PaproF_PaproM_methylation_stats.txt'
with open(stats_path, 'w') as fout:
    fout.write('Paraphymatoceros proskaueri — per-site 5mC methylation statistics\n')
    ...
    for glabel in _genome_labels:
        ...
        for cat in CATS:
            ...
            if len(c) == 0: fout.write(f'  {cat:16s}  {"0":>10}\n'); continue
...
fig4.suptitle('Paraphymatoceros proskaueri — autosomal CpG methylation distribution\n'
              'per-site % modified  ·  min_cov ≥ 5×  ·  autosomes only',
              fontsize=11, fontweight='bold')
plt.tight_layout()
dist_out = 'PaproF_PaproM_methylation_distribution'
