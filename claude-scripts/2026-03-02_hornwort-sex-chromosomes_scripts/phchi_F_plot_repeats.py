"""
Repeat composition visualization for PhchiF (Phaeomegaceros chiloensis female, 14765-5)
...
OUT   = f'{BASE}/PhchiF_repeat_composition'
...
gs_mid = gridspec.GridSpec(1, 1, figure=fig,
                           left=0.07, right=0.97, top=0.71, bottom=0.55)
gs_bot = gridspec.GridSpec(n_scaf, 1, figure=fig,
                           left=0.07, right=0.97, top=0.50, bottom=0.04,
                           hspace=0.07)
...
ax_bar.set_xticks(x)
ax_bar.set_xticklabels(SLABELS, fontsize=7)
...
for ax_i, (ax, seqid) in enumerate(zip(axs_win, SCAFFOLDS)):
    ...
    suffix = '  [U chromosome]' if is_sex else ''
    ax.text(0.005, 0.90,
...
fig.text(0.5, 0.995,
         'Phaeomegaceros chiloensis female (PhchiF, 14765-5)  —  Repetitive element composition',
