"""
Male vs. female genome comparison: Paraphymatoceros proskaueri (PaproF vs PaproM)
Emphasis on sex chromosome differentiation (U chromosome in female S5, V chromosome in male S5)
...
FEMALE_DIR = f'{BASE}/Papro252-9.2.2/final_genome_prep'
MALE_DIR   = f'{BASE}/Papro252-3/final_genome_prep'
F_BED      = f'{BASE}/Papro252-9.2.2/methylation/PaproF_genome.5mCG_5hmCG.bed'
M_BED      = f'{BASE}/Papro252-3/methylation/PaproM_genome.5mCG_5hmCG.bed'
OUT        = f'{BASE}/sex_chromosome_analyses/PaproF_PaproM_sex_chromosome_comparison'

METHYL_MIN_COV = 5
METHYL_COLOR   = '#6a3d7a'   # muted violet for 5mC
HMETH_COLOR    = '#b090c0'   # soft mauve for 5hmC

F_SCAFFOLDS = ['PaproF.S1', 'PaproF.S2', 'PaproF.S3', 'PaproF.S4', 'PaproF.S5']
F_SEX       = 'PaproF.S5'   # U chromosome (female)

M_SCAFFOLDS = ['PaproM.S1', 'PaproM.S2', 'PaproM.S3', 'PaproM.S4', 'PaproM.S5']
M_SEX       = 'PaproM.S5'   # V chromosome (male)
...
def load_methylation_means(bed_file, scaffolds, min_cov=5):
    """Stream bedMethyl file and return per-scaffold mean 5mC and 5hmC.

    Returns {seqid: (mC_mean_pct, hmC_mean_pct)}.
    """
    sums_m = {s: 0.0 for s in scaffolds}
    sums_h = {s: 0.0 for s in scaffolds}
    ns_m   = {s: 0   for s in scaffolds}
    ns_h   = {s: 0   for s in scaffolds}
    ...
    result = {}
    for seqid in scaffolds:
        mC  = sums_m[seqid] / ns_m[seqid]  if ns_m[seqid]  > 0 else float('nan')
        hmC = sums_h[seqid] / ns_h[seqid]  if ns_h[seqid]  > 0 else float('nan')
        result[seqid] = (mC, hmC)
    return result
...
f_genome = load_genome(f'{FEMALE_DIR}/pilon.3_renamed.fasta')
print('Loading female repeats...')
f_rep_major, f_all_rep = load_repeats(f'{FEMALE_DIR}/pilon.3_renamed.fasta.mod.EDTA.TEanno.gff3')
print('Loading female genes...')
f_genes, f_exons = load_genes(f'{FEMALE_DIR}/braker_renamed.gtf')

print('Loading male genome...')
m_genome = load_genome(f'{MALE_DIR}/PaproM.renamed_renamed.fasta')
print('Loading male repeats...')
m_rep_major, m_all_rep = load_repeats(f'{MALE_DIR}/PaproM.renamed_renamed.fasta.mod.EDTA.TEanno.gff3')
print('Loading male genes...')
m_genes, m_exons = load_genes(f'{MALE_DIR}/braker_renamed.gtf')
...
print('Loading female methylation...')
f_meth = load_methylation_means(F_BED, F_SCAFFOLDS, METHYL_MIN_COV)
print('Loading male methylation...')
m_meth = load_methylation_means(M_BED, M_SCAFFOLDS, METHYL_MIN_COV)

# Autosomal methylation averages (size-weighted)
def auto_meth_avg(scaffolds, sex_scaf, genome, meth_means):
    autos      = [s for s in scaffolds if s != sex_scaf]
    total_size = sum(genome[s] for s in autos)
    mC_w  = sum(meth_means[s][0] * genome[s] for s in autos) / total_size
    hmC_w = sum(meth_means[s][1] * genome[s] for s in autos) / total_size
    return mC_w, hmC_w

f_auto_meth = auto_meth_avg(F_SCAFFOLDS, F_SEX, f_genome, f_meth)
m_auto_meth = auto_meth_avg(M_SCAFFOLDS, M_SEX, m_genome, m_meth)
f_sex_meth  = f_meth[F_SEX]
m_sex_meth  = m_meth[M_SEX]
print(f'  F autosomes: 5mC={f_auto_meth[0]:.1f}%  U chr: 5mC={f_sex_meth[0]:.1f}%')
print(f'  M autosomes: 5mC={m_auto_meth[0]:.1f}%  V chr: 5mC={m_sex_meth[0]:.1f}%')
...
gs_top = gridspec.GridSpec(1, 2, figure=fig,
                           left=0.08, right=0.97, top=0.95, bottom=0.77,
                           wspace=0.32)
gs_mid = gridspec.GridSpec(1, 1, figure=fig,
                           left=0.08, right=0.97, top=0.71, bottom=0.52)
gs_bot = gridspec.GridSpec(1, 2, figure=fig,
                           left=0.08, right=0.97, top=0.46, bottom=0.26,
                           wspace=0.32)
gs_new = gridspec.GridSpec(1, 1, figure=fig,
                           left=0.08, right=0.97, top=0.21, bottom=0.05)
...
ax_F = fig.add_subplot(gs_new[0, 0])
...
# Both sex chromosomes are S5 in Papro
f_xlabels = ['S1', 'S2', 'S3', 'S4', 'S5\n(U)']
...
ax_A.text(np.mean(f_x), ymax_a * 0.99, 'Female (PaproF)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=F_AUTO)
ax_A.text(np.mean(m_x), ymax_a * 0.99, 'Male (PaproM)',
...
ax_B.text(np.mean(f_x), 108, 'Female (PaproF)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=F_AUTO)
ax_B.text(np.mean(m_x), 108, 'Male (PaproM)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=M_AUTO)
...
ax_C.scatter(f_sex_rep, f_sex_gen, c=F_SEX_C, s=9, alpha=0.55, linewidths=0, zorder=4,
             label=f'Female S5 / U chromosome  (n = {len(f_sex_rep):,} windows)')
ax_C.scatter(m_sex_rep, m_sex_gen, c=M_SEX_C, s=9, alpha=0.60, linewidths=0, zorder=4,
             label=f'Male S5 / V chromosome  (n = {len(m_sex_rep):,} windows)')
...
ax_E.legend(fontsize=7, ncol=2, loc='upper left', frameon=False,
            handlelength=0.9, handletextpad=0.4, columnspacing=0.8)
ax_E.text(-0.14, 1.08, 'E', transform=ax_E.transAxes,
          fontsize=11, fontweight='bold', va='top')

# ── F: CpG methylation — 4-group comparison ─────────────────────────────────
# Groups: F autosome mean | F U-chr | M autosome mean | M V-chr
f_grp_labels = ['Autosome\nmean (F)', 'U chr\n(F-S5)', 'Autosome\nmean (M)', 'V chr\n(M-S5)']
f_grp_colors = [F_AUTO, F_SEX_C, M_AUTO, M_SEX_C]
mC_grp  = [f_auto_meth[0], f_sex_meth[0], m_auto_meth[0], m_sex_meth[0]]
hmC_grp = [f_auto_meth[1], f_sex_meth[1], m_auto_meth[1], m_sex_meth[1]]

f_d_x  = np.arange(4)
bw_f   = 0.60

bars_f = ax_F.bar(f_d_x, mC_grp, bw_f, color=f_grp_colors, linewidth=0, zorder=3,
                  label='5mC methylation')
for bar, val in zip(bars_f, mC_grp):
    ax_F.text(bar.get_x() + bar.get_width() / 2,
              bar.get_height() + 1.0,
              f'{val:.1f}%', ha='center', va='bottom', fontsize=8, color='#333333')

ax_F.axvline(1.5, color='#cccccc', linewidth=0.8, linestyle='--', zorder=0)
ax_F.set_xticks(f_d_x)
ax_F.set_xticklabels(f_grp_labels, fontsize=8.5)
for tick, col in zip(ax_F.get_xticklabels(), f_grp_colors):
    tick.set_color(col)
ax_F.set_ylim(0, 100)
ax_F.set_ylabel('Mean 5mC (%)', fontsize=8)
style_ax(ax_F)
ax_F.set_title('CpG methylation: autosome mean vs. sex chromosome',
               fontsize=8.5, pad=5)
ax_F.text(0.5, 103, 'Female', ha='center', va='bottom', fontsize=7.5,
          fontweight='bold', color=F_AUTO, clip_on=False)
ax_F.text(2.5, 103, 'Male', ha='center', va='bottom', fontsize=7.5,
          fontweight='bold', color=M_AUTO, clip_on=False)
...
ax_F2 = ax_F.twinx()
ax_F2.plot(f_d_x, hmC_grp, 'o-', color=HMETH_COLOR, lw=1.4, ms=6,
           zorder=5, label='5hmC methylation')
for xi, val in zip(f_d_x, hmC_grp):
    ax_F2.text(xi + 0.35, val + 0.1, f'{val:.1f}%',
               ha='left', va='bottom', fontsize=7, color=HMETH_COLOR)
ax_F2.set_ylim(0, 10)
ax_F2.set_yticks([0, 5, 10])
ax_F2.tick_params(axis='y', labelsize=6.5, colors=HMETH_COLOR,
                  length=2.5, width=0.5)
ax_F2.set_ylabel('Mean 5hmC (%)', fontsize=7, color=HMETH_COLOR, labelpad=2)
ax_F2.spines['right'].set_color(HMETH_COLOR)
ax_F2.spines['right'].set_linewidth(0.5)
ax_F2.spines['top'].set_visible(False)

h_mC  = mpatches.Patch(color=F_AUTO, label='5mC methylation', linewidth=0)
h_hmC = Line2D([0], [0], color=HMETH_COLOR, marker='o', lw=1.4, ms=6,
               label='5hmC methylation (right axis)')
ax_F.legend(handles=[h_mC, h_hmC], fontsize=7.5, loc='upper right',
            frameon=False, handlelength=1.0, handletextpad=0.5)

fig.text(0.5, 0.993,
         'Paraphymatoceros proskaueri  —  Female vs. male genome comparison',
         ha='center', va='top', fontsize=11, fontweight='bold', color='#1a1a1a')
fig.text(0.5, 0.979,
         'U chromosome (female S5)  ·  V chromosome (male S5)',
