"""
Male vs. female genome comparison: Phymatoceros phymatodes (PhphyF vs PhphyM)
...
FEMALE_DIR = f'{BASE}/Phymato_6/final_genome_prep'
MALE_DIR   = f'{BASE}/Phymato_ref_genome/final_genome_prep'
OUT        = f'{BASE}/sex_chromosome_analyses/PhphyF_PhphyM_sex_chromosome_comparison'

F_SCAFFOLDS = ['PhphyF.S1', 'PhphyF.S2', 'PhphyF.S3', 'PhphyF.S4', 'PhphyF.S5']
F_SEX       = 'PhphyF.S5'   # U chromosome (female)

M_SCAFFOLDS = ['PhphyM.S1', 'PhphyM.S2', 'PhphyM.S3', 'PhphyM.S4', 'PhphyM.S5']
M_SEX       = 'PhphyM.S5'   # V chromosome (male)
...
f_genome = load_genome(f'{FEMALE_DIR}/PhphyF_genome.fasta')
print('Loading female repeats...')
f_rep_major, f_all_rep = load_repeats(f'{FEMALE_DIR}/PhphyF_repeat_annotations.gff')
print('Loading female genes...')
f_genes, f_exons = load_genes(f'{FEMALE_DIR}/braker_renamed.gtf')

print('Loading male genome...')
m_genome = load_genome(f'{MALE_DIR}/PhphyM_genome.fasta')
print('Loading male repeats...')
m_rep_major, m_all_rep = load_repeats(f'{MALE_DIR}/pilon.3_renamed.fasta.mod.EDTA.TEanno.gff3')
...
# Both sex chromosomes are S5 in Phymato
...
ax_A.text(np.mean(f_x), ymax_a * 0.99, 'Female (PhphyF)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=F_AUTO)
ax_A.text(np.mean(m_x), ymax_a * 0.99, 'Male (PhphyM)',
...
ax_B.text(np.mean(f_x), 108, 'Female (PhphyF)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=F_AUTO)
ax_B.text(np.mean(m_x), 108, 'Male (PhphyM)',
          ha='center', va='top', fontsize=7.5, fontweight='bold', color=M_AUTO)
...
fig.text(0.5, 0.990,
         'Phymatoceros phymatodes  —  Female vs. male genome comparison',
