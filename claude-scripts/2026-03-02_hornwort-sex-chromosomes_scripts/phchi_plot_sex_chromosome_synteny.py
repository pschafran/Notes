#!/usr/bin/env python3
"""
U/V sex chromosome synteny plot for Phaeomegaceros chiloensis.

Draws gene tick marks for all genes on the U (PhchiF.S4) and V (PhchiM.S6)
...
# ── paths ────────────────────────────────────────────────────────────────────
BASE    = '/media/data/projects/hornwort_sex_chromosomes/analysis/Phaeomegaceros_fimbriatus/sex_chromosome_analyses'
GTF_F   = '/media/data/projects/hornwort_sex_chromosomes/analysis/Phaeomegaceros_fimbriatus/Phaeomegaceros_14765-5/final_genome_prep/PhchiF_gene_annotations.gtf'
GTF_M   = '/media/data/projects/hornwort_sex_chromosomes/analysis/Phaeomegaceros_fimbriatus/Phaeomegaceros_14765-4/final_genome_prep/PhchiM_gene_annotations.gtf'
TSV     = f'{BASE}/rbh_ks/PhchiF_PhchiM_RBH_Ks.tsv'
OUT_PDF = f'{BASE}/rbh_ks/PhchiF_PhchiM_sex_chromosome_synteny.pdf'
OUT_PNG = f'{BASE}/rbh_ks/PhchiF_PhchiM_sex_chromosome_synteny.png'
OUT_CAP = f'{BASE}/rbh_ks/PhchiF_PhchiM_sex_chromosome_synteny.caption.txt'
...
def strip_transcript_suffix(tid):
    """PhchiF.S4G000300.t1 → PhchiF.S4G000300"""
...
def load_rbh(tsv_file):
    ...
    Return three lists of dicts:
      uv_pairs  — scaffold_F=PhchiF.S4, scaffold_M=PhchiM.S6, flag=ok
      u_auto    — scaffold_F=PhchiF.S4, scaffold_M!=PhchiM.S6
      v_auto    — scaffold_M=PhchiM.S6, scaffold_F!=PhchiF.S4
    """
    ...
    with open(tsv_file) as fh:
        ...
        for line in fh:
            ...
            rec = dict(gene_F=gene_F, gene_M=gene_M,
                       ...
                       Ks=ks, flag=flag)
            if scaf_F == 'PhchiF.S4' and scaf_M == 'PhchiM.S6' and flag == 'ok':
                uv_pairs.append(rec)
            elif scaf_F == 'PhchiF.S4' and scaf_M != 'PhchiM.S6':
                u_auto.append(rec)
            elif scaf_M == 'PhchiM.S6' and scaf_F != 'PhchiF.S4':
                v_auto.append(rec)
...
u_genes = load_gene_midpoints(GTF_F, 'PhchiF.S4')
v_genes = load_gene_midpoints(GTF_M, 'PhchiM.S6')
...
ax.text(-0.02, (U_Y0 + U_Y1) / 2, 'U (PhchiF.S4)\n16.3 Mb',
        va='center', ha='right', fontsize=9,
        color='#333333', linespacing=1.4)
ax.text(-0.02, (V_Y0 + V_Y1) / 2, 'V (PhchiM.S6)\n6.2 Mb',
        va='center', ha='right', fontsize=9,
        color='#333333', linespacing=1.4)

# ── Mb tick labels on chromosomes ─────────────────────────────────────────────
def add_mb_ticks(ax, chrom_len, y_bar_top, y_bar_bot, above=True, mb_step=2):
...
add_mb_ticks(ax, LEN_U, U_Y1, U_Y0, above=True,  mb_step=2)
add_mb_ticks(ax, LEN_V, V_Y0, V_Y1, above=False, mb_step=1)
...
ax.set_title('Sex chromosome synteny — U (female) vs V (male)\n'
             'Phaeomegaceros chiloensis (PhchiF.S4 × PhchiM.S6)',
...
ks_lo = min(ks_vals) if ks_vals else float('nan')
ks_hi = max(ks_vals) if ks_vals else float('nan')

caption = f"""\
Figure. Synteny of gene-bearing regions on the U (female) and V (male) sex chromosomes of
Phaeomegaceros chiloensis. The U chromosome (PhchiF.S4, 16.3 Mb; top bar) and V
chromosome (PhchiM.S6, 6.2 Mb; lower bar) are each drawn to scale along a normalised
horizontal axis. Vertical tick marks above/below each bar indicate the midpoint position of
every annotated gene ({len(u_genes)} on U, {len(v_genes)} on V). Bezier arcs connect the
...
{ks_lo:.2f}–{ks_hi:.2f}). Ticks coloured dark grey mark genes with a cross-sex RBH to an
autosome of the other individual ({n_u_auto} U-chromosome genes → male autosomes;
{n_v_auto} V-chromosome genes → female autosomes). Ks was estimated using the YN00 method
