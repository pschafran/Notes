#!/bin/bash
# check_odp_files_liverworts.sh
# Validates protein/chrom/genome file consistency for ODP liverwort species.
...
#
# Usage: bash check_odp_files_liverworts.sh [BASE_DIR]
# Default BASE_DIR: /media/data/projects/hornwort_sex_chromosomes/analysis/synteny/odp_liverworts_20260401

BASE=${1:-/media/data/projects/hornwort_sex_chromosomes/analysis/synteny/odp_liverworts_20260401}

SPECIES="Mapol Maqua Ricav Riflu Rinat Acrsan Blpus Concon Junere Radobc"
...
PYEOF
  ...
  if [ -n "$removed_names" ]; then
    ...
    echo "  WARNING: $sp — protein and chrom files updated; originals saved to .bak"
  elif [ -n "$stop_report" ]; then
    echo "  WARNING: $sp — protein file updated (trailing stops truncated); original saved to .bak"
  fi
